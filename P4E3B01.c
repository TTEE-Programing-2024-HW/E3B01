...
//心得：
//經過這次的程式設計練習，我學到了很多有關 C 語言的知識與技巧。
//這次的作業是設計一個成績管理系統，讓我更加了解如何使用結構體來處理複雜的數據。
//一開始在撰寫程式時，我遇到了許多困難，特別是在輸入學生成績的功能上。
//由於學號和成績的範圍有限制，我必須不斷檢查使用者輸入的有效性，這讓我學會了如何使用 if 條件語句來處理多種情況。
